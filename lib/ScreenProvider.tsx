...
let currentBreakpoint: Breakpoint = getBreakpointByScreenWidth(windowDimensions.width);
...
export function ScreenProvider({ children }: ScreenProviderProps) {
  ...
  const [fontScaleFactor, setFontScaleFactor] = useState(windowDimensions.fontScale)

  const handleScreenResize = useCallback(({ window }: { window: ScaledSize }) => {
