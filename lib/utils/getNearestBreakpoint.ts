...
export function getNearestBreakpoint({
  ...
  const previousBreakpoints = breakpoints.filter((_, index) => index < breakpointIndex);

  const nearestBreakpoint = previousBreakpoints
    .reverse()
    .find(findBreakpoint => {
      return availableBreakpoints.some(availableBreakpoint => findBreakpoint.size === availableBreakpoint)
    })
